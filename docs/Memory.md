...
## 2026-09-02 — Dark Mode, Subregion Comparison & Micro-Animations Added
- **Done:**
  1. **🌓 Dark Mode / Light Mode Switcher:** Added theme toggle button in navbar with custom CSS tokens (`#0B0C16` canvas, `#17182B` surface cards, glowing brand accents) and `localStorage` persistence.
  2. **⚖️ Side-by-Side Subregion Comparison Tool:** Added a dedicated tab (`#tab-comparison`) allowing dynamic comparison of two subregions (CBD vs South Auckland, North Shore vs West Auckland) across Orders, Revenue, Profit, Margin, High-Risk Count, Delivery Radius, and a dual-series channel share bar chart.
  3. **✨ Interactive Tooltip Cards & Animations:** Added smooth hover lifts, pulsating status badges, interactive tooltips, and fade-in transitions.
- **Status:** All features complete, verified, and running on `http://localhost:8000`.
